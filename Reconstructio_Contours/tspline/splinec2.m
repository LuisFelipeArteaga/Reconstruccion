function [x, y, l] = splinec2 (xi, yi, data, t, cutoff, n_out)
%	$Id: splinec2.m,v 1.1.1.1 2005/01/26 00:45:57 pwessel Exp $
% SPLINEC2	Curve-in-plane interpolation using Green's function for a spline in tension
%
%	SPLINEC2 will find a spline-based curve using continuous curvature splines
...
%	the x-y plane
%
%	[x, y]    = splinec2 (xi, yi, data, t, cutoff, n_out)
%	[x, y, l] = splinec2 (xi, yi, data, t, cutoff, n_out)
%
% The input parameters are:
%
%	xi, yi	- coordinates of data constraints
...
%
%	x, y	- the interpolating planer curve
...
s = zeros (size (xi));
for i=2:n
	s(i) = s(i-1) + sqrt ((xi(i) - xi(i-1))^2 + (yi(i) - yi(i-1))^2);
end
...
% make the call to spline1d with 2 rhs

if (nargout == 2)
	xy = spline1d (s_out, s(k_pts), [xi(k_pts) yi(k_pts)], s(k_slp), [cos(data(k_slp)) sin(data(k_slp))], t, cutoff);
elseif (nargout == 3)
	[xy, l] = spline1d (s_out, s(k_pts), [xi(k_pts) yi(k_pts)], s(k_slp), [cos(data(k_slp)) sin(data(k_slp))], t, cutoff);
else
	[xy, l, A] = spline1d (s_out, s(k_pts), [xi(k_pts) yi(k_pts)], s(k_slp), [cos(data(k_slp)) sin(data(k_slp))], t, cutoff);
end
x = xy(:,1);
y = xy(:,2);
