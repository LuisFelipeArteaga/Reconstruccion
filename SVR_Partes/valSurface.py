...
def FindParamSVR(X,t,Kernel,t_data,ii):
    ...
    num_Workers = 2
    nP = 10
    Train_Index2,Test_Index2 = crossval_val(X,t,nP)
    #-------------------------------------------------
    ...
    for ff in range(nP):
        ...
        filename2 = os.path.join('Experimento_1_Superior/data_save','Error','error'+str(ii)+'_'+str(ff)+'.npy')
...
if __name__ == '__main__':
    ...
    nPart = 10
    filename = os.path.join('Point_Cloud','MycSuperior.ply')
    #fepcd1  = opd.read_point_cloud(filename)
    
    X,t = get_data_myc(filename)
   
    #---------------------------------------------------------
    #Validation
    Train_Index,Test_Index = crossval_val(X,t,nPart)
    print(X.shape[0]/3)
    #---------------------------------------------------------
...
#      
        ...
        filename2 = os.path.join('Experimento_1_Superior/Model','svrMycInferior'+str(ii+1)+'.joblib')
...
#      
        np.save('Experimento_1_Superior/data_save/cDic.npy',cDic)
        np.save('Experimento_1_Superior/data_save/gammaDic.npy',gammaDic)
        np.save('Experimento_1_Superior/data_save/epsilonDic.npy',epsilonDic)
        np.save('Experimento_1_Superior/data_save/errorDic.npy',errorDic)
        np.save('Experimento_1_Superior/data_save/error2Dic.npy',errorList2)
