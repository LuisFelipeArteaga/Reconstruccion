function [Transform, C, T,Y] = ECPD_Algorithm(X, Y, opt)
...
[C, W, sigma2, iter, T] =ECPD(X, Y, opt);
...
%% ECPD
function [C, W, sigma2, iter, T] =ECPD(X, Y, opt)
%Parametros
beta  = opt.beta;
lambda = opt.lambda;
max_it  = opt.max_it;
tol  = opt.tol;
outliers = opt.outliers; %w
corresp = opt.corresp;
sigma2 = opt.sigma2;
alpha = opt.alpha;
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 1e-8)
    ...
    %% M-step
    dP = spdiags(P1,0,M,M);
    W = (dP*G + (sigma2/(alpha*alpha))*G + lambda*sigma2*eye(M))...
        \(PX-dP*T + (sigma2/(alpha*alpha))*(PX-dP*T));
  
    %Actualizar posiciones
    T=Y+G*W;
    Np=sum(P1);
    dPt = spdiags(Pt1,0,N,N);
    sigma2 = (trace(X'*dPt*X) - 2*trace(PX'*T) + trace(T'*dP*T))/(Np*D);
    %% Plot iteraciones
    if 1
